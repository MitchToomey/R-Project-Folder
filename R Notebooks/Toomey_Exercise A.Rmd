...
### 6
#I would use the attribute function. 

### 7
```{r}
attr(distance, "emails") <-c ("ademir@email.sc.edu","ojlewis@email.sc.edu", "gonzalsk@email.sc.edu" ,"binhazza@email.sc.edu", "tt12@email.sc.edu", "walterc5@email.sc.edu")
```
...
colnames(correlation_matrix) <- c("ID", "Brain Size", "Height")
...
names(restaurant_info) <- c("Restaurant Name", "City", "Open Hours",
                        "Type of Cuisine", "Acohol?", "Entree Price Range")
```
...
sum(distance_greater_5)

# Everyone on my list's total miles from campus is 93 miles. 
```
...
t_result <- c(t.test(Student_Data$Score ~ Student_Data$Gender))
```
...
z_scores <- (Student_Data$Score-mean(Student_Data$Score))/sd(Student_Data$Score)

z_scores

cbind(Student_Data, z_scores)
```
